...
echo "=== INSTALL SCRIPT FOR 3007X CONTROL PANEL ==="
...
# -----------------------------
# 4. ติดตั้งคำสั่งลัดใหม่เป็น 3007x
# -----------------------------
TARGET="$PREFIX/bin/3007x"  # <--- เปลี่ยนชื่อไฟล์ระบบปลายทางตรงนี้
SOURCE="setup/SMS-RUN-WEB-TERMUX.sh"

if [ ! -f "$SOURCE" ]; then
    echo "[!] ไม่พบไฟล์ $SOURCE สำหรับสร้างคำสั่งรัน"
else
    echo "[*] กำลังติดตั้งคำสั่งลัด 3007x เข้าสู่ระบบ..."
    mv "$SOURCE" "$TARGET"
    chmod +x "$TARGET"
    echo "[OK] ติดตั้งคำสั่ง 3007x สำเร็จ!"
fi
...
echo "   พิมพ์คำสั่ง: 3007x"  # <--- เปลี่ยนคู่มือแจ้งเตือนให้เป็นคำสั่งใหม่
